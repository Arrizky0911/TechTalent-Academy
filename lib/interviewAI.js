import {
  feedbackOutputParser,
  formatInterviewResult,
  jsonParser,
} from "./AIFormat";
import { addInterviewHistory, vectorSearch } from "./AstraDBConfig";
import { getResponse } from "./GoogleAIConfig";
import { randomUUID } from "expo-crypto";

export const getQuestions = async (job) => {
  const retrieval = await vectorSearch(
...
export const getFeedbacks = async (questions, answers, userID, job) => {
  let session = randomUUID();
  let prompt = formatInterviewResult(questions, answers ,job);
  const requestContent = [
    {
      role: "user",
      parts: [
  ...
  try {
    ...
    await addInterviewHistory(output, output[0].grade, userID, session, job)
    return output;
  } catch (error) {
    throw new Error(error);
  }
